...
// a kindbnkctl PoC: bring the local stack up if it isn't already, then
...
// this with errors.Is; explicit operator invocations (`kindbnkctl
// bnk-forge launch`) propagate it as a regular error.
//
// kindbnkctl never installs bnk-forge for the operator: if the stack
...
// the subset kindbnkctl populates is here.
type Project struct {
...
// YAML body of the localized kubeconfig kindbnkctl writes to
