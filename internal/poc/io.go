...
func New(name string) *PoC {
	return &PoC{
		...
		Versions: Versions{
			...
			KindNodeImage: version.K8sNodeImage,
		...
		Networks: Networks{
			Internal: DockerNetwork{
				Name:   version.DockerNetworkInternal,
				Subnet: version.DefaultInternalSubnet,
			},
			External: DockerNetwork{
				Name:   version.DockerNetworkExternal,
				Subnet: version.DefaultExternalSubnet,
			},
		},
		BNK: BNK{
...
func (p *PoC) Save(dir string) error {
	...
	header := []byte("# kindbnkctl PoC declarative state — source of truth for this PoC.\n" +
